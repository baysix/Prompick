 ...
 * 내 작업함.
 *
 * 진행 중인 작업이 있으면 목록을 계속 새로고침한다. 사용자가 이 화면을 열어두고 기다리는
 * 경우가 많기 때문이다. 모두 끝났으면 멈춘다.
 */
        ...
        {TABS.map((t, i) => (
          <button
            ...
            className={cn(
              ...
              i === tab ? "bg-ink font-semibold text-ground" : "text-ink-soft hover:bg-surface",
            )}
          >
            {t.label}
          </button>
      ...
      <Link href={`/jobs/${job.id}`} className="group block">
        <div className="relative aspect-[9/16] overflow-hidden rounded-xl bg-surface">
          ...
          {!isFinished(job.status) && (
            <span className="absolute left-2 top-2 rounded-full bg-accent px-2 py-0.5 text-[10px] font-semibold text-accent-ink">
              만드는 중
            </span>
          )}
          {job.status === "FAILED" && (
            <span className="absolute left-2 top-2 rounded-full bg-surface-2 px-2 py-0.5 text-[10px] font-medium text-ink-soft">
              실패
            </span>
          )}
        </div>
